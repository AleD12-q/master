...
public interface LemmaRepository extends JpaRepository<Lemma, Integer> {
    ...
    @Query(value = "select frequency from Lemma l where l.lemma = :textLemma and l.site = :site ")
    Optional<Integer> findFrequencyByTextLemmaAndSite(
            @Param("textLemma") String textLemma, @Param("site") Site site);
    @Query(value = "select sum(l.frequency) from Lemma l join Site s on s = l.site and s.type = 'INDEXED' and l.lemma =  :textLemma group by l.lemma")
